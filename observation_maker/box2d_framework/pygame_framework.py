...
class PygameDraw(b2DrawExtended):
    ...
    def DrawSegment(self, p1, p2, color, drawwidth=LINE_WIDTH):
        """
        Draw the line segment from p1-p2 with the specified color.
        """

        pygame.draw.line(self.surface, color.bytes, p1, p2, drawwidth)
    ...
    def DrawPolygon(self, vertices, color):
        """
        Draw a wireframe polygon given the screen
        vertices with the specified color.
        """
        if not vertices:
            return
    ...
    def DrawSolidPolygon(self, vertices, color):
        """
        Draw a filled polygon given the screen
        vertices with the specified color.
        """
        if not vertices:
            return
    ...
    #     In the class instance, we store a zoom factor, an offset indicating
    #     where the view extents start at, and the screen size (in pixels).
    #     """
...
class PygameFramework(FrameworkBase):
    ...
    viewZoom = property(
        lambda self: self._viewZoom, setZoom,
        doc='Zoom factor for the display')
    viewCenter = property(
        lambda self: self._viewCenter / self._viewZoom, setCenter,
    ...
    def checkEvents(self):
        ...
        for event in pygame.event.get():
            if event.type == QUIT \
               or (event.type == KEYDOWN and event.key == Keys.K_ESCAPE):
                return False
    ...
    def _Keyboard_Event(self, key, down=True):
        ...
        Checks for the initial keydown of the basic testbed keys.
        Passes the unused ones onto the test via the Keyboard() function.
        """
        if down:
    ...
    def Step(self, settings):
        ...
        if GUIEnabled:
            # In case during the step the settings changed, update the GUI
            # reflecting those settings.
            self.gui_table.updateGUI(self.settings)
    ...
    def Print(self, str, color=(255, 255, 255, 255)):
        """
