...
class FrameworkBase(b2ContactListener):
    ...
    * Do NOT want to implement your own renderer:
    ...
    def Step(self, settings):
        ...
        Takes care of physics drawing (callbacks areexecuted after the
        ...
        if renderer:
            ...
            if settings.drawFPS:
                self.Print("Combined FPS %.2f" % self.fps)
    ...
    def LaunchBomb(self, position, velocity):
        """
        A bomb is a simple circle which has the specified position
        and velocity. position and velocity must be b2Vec2's.
        """
        if self.bomb:
            self.world.DestroyBody(self.bomb)
            self.bomb = None
    ...
    # These can/should be implemented in the test subclass: (Step() also
    # if necessary). See empty.py for a simple example.
...
def main(test_class, single_step=False):
    """
    ...
    if fwSettings.onlyInit:
        return
    return test.run(single_step), test
